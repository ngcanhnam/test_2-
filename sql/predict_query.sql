...
WHERE trading_date BETWEEN '{{predict_start}}' AND '{{predict_end}}'
...
SELECT
    ...
    COALESCE(
      DATEDIFF(YEAR, MIN(open_date), '{{predict_end}}')
      + (DATEDIFF(MONTH, MIN(open_date), '{{predict_end}}')
         - DATEDIFF(YEAR, MIN(open_date), '{{predict_end}}')*12)/12.0,
      0.0
    ) AS open_time
INTO #customer_open
...
FROM master_dds.fact.fact_customer_daily
WHERE report_date BETWEEN '{{predict_start}}' AND '{{predict_end}}'
GROUP BY
    master_account,
...
WHERE c_order_date BETWEEN '{{predict_start}}' AND '{{predict_end}}'
GROUP BY
